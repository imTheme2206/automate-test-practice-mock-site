...
class Store {
  ...
  private seedData() {
    ...
    const demoPost1: Post = {
      ...
      content: 'This is a mock Reddit clone designed for practicing automated testing. Feel free to create accounts, posts, and comments. All data resets on page reload!',
      authorId: 'user-1',
    ...
    const demoPost2: Post = {
      ...
      content: 'When writing Selenium tests, always use explicit waits instead of implicit waits. Use data-testid attributes for reliable element selection. Keep your tests independent and isolated.',
      authorId: 'user-2',
    ...
    const demoPost3: Post = {
      ...
      content: 'I\'m struggling with testing pages that have dynamic content loaded via AJAX. What strategies do you use? Polling? WebSocket monitoring? Would love to hear your approaches.',
      authorId: 'user-1',
    ...
    const demoComment1: Comment = {
      id: 'comment-1',
      content: 'Great resource for learning automation testing! Thanks for setting this up.',
      authorId: 'user-2',
      postId: 'post-1',
      parentId: null,
      createdAt: new Date(Date.now() - 1 * 24 * 60 * 60 * 1000),
    ...
    const demoComment2: Comment = {
      id: 'comment-2',
      content: 'I always use data-testid, it makes selectors so much more reliable.',
      authorId: 'user-1',
      postId: 'post-2',
      parentId: null,
  ...
  // Post methods
  createPost(title: string, content: string): { success: boolean; error?: string; post?: Post } {
  ...
  votePost(postId: string, voteType: 'up' | 'down'): { success: boolean; error?: string } {
    if (!this.auth.currentUser) {
  ...
  // Comment methods
  createComment(postId: string, content: string, parentId: string | null = null): { success: boolean; error?: string; comment?: Comment } {
    if (!this.auth.currentUser) {
  ...
  voteComment(commentId: string, voteType: 'up' | 'down'): { success: boolean; error?: string } {
    if (!this.auth.currentUser) {
      return { success: false, error: 'You must be logged in to vote' };
    }
